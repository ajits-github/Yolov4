...
#                 print('use_darknet_cfg___________________________')
#                 eval_model = Darknet(cfg.cfgfile, inference=True)
#             else:
#                 print('Yolov4___________________________')
#                 eval_model = Yolov4(cfg.pretrained, n_classes=cfg.classes, inference=True)
#             # eval_model = Yolov4(yolov4conv137weight=None, n_classes=config.classes, inference=True)
#             if torch.cuda.device_count() > 1:
#                 print("MULTIPLE CUDA IN USE: ",torch.cuda.device_count())
#                 eval_model.load_state_dict(model.module.state_dict())
#             else:
#                 print("SINGLE CUDA IN USE: ",torch.cuda.device_count())
...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=50) as pbar:
            ...
            eval_model.to(device)
            # evaluator = evaluate(eval_model, val_loader, config, device)
            evaluator = evaluate(eval_model, val_loader, config, device, epoch, epochs,n_val)
            del eval_model
...
def evaluate(model, data_loader, cfg, device, epoch, epochs, n_val, logger=None, **kwargs):
    ...
    # print("Before  CocoEvaluator train.py================================")
    coco_evaluator = CocoEvaluator(coco, iou_types = ["bbox"], bbox_fmt='coco')
    with tqdm(total=n_val, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=50) as pbar:
...
def get_args(**kwargs):
    ...
    parser.add_argument('-e','--epochs',dest='TRAIN_EPOCHS', type=int, default=200)
    ...
    parser.add_argument('-keep-checkpoint-max', type=int, default=10, help='maximum number of checkpoints to keep. If set 0, all checkpoints will be kept',
...
if __name__ == "__main__":
    ...
    print("CFG in train.py::::::::::::: ",cfg)
    print("\n")
    print("Train_label in train.py______________",cfg.train_label)
    print("val_label in train.py______________",Cfg.val_label)
    print("\n")
    ...
    if torch.cuda.device_count() > 1:
        ...
        print("multiple cuda:----------",torch.cuda.device_count())
        # print("model:====",model)
    ...
    #     print("multiple cuda:----------",torch.cuda.device_count())
    #     # print("model:====",model)
    #     device = select_device(cfg.gpu, batch_size=cfg.batch)
    #     dist.init_process_group(backend='nccl', init_method='env://')  # distributed backend
    #     assert cfg.batch % cfg.world_size == 0, '--batch-size must be multiple of CUDA device count'
    #     cfg.batch = cfg.batch // cfg.world_size
